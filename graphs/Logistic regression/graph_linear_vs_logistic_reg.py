...
ggplot(data=df, mapping=aes(x="x", y="y")) + geom_point(colour="blue") + stat_smooth(
    method="glm", se=False, colour="red", size=0.5
) + scale_y_continuous(
    limits=[-0.2, 1.2], breaks=[0, 1], labels=["0", "1"]
) + theme_classic() + labs(
    x="X", y="Y"
)


ggplot(data=df, mapping=aes(x="x", y="y")) + geom_point(colour="blue") + geom_point(
    aes(y="preds"), colour="red", size=0.5
) + scale_y_continuous(
    limits=[-0.2, 1.2], breaks=[0, 1], labels=["0", "1"]
) + theme_classic() + labs(
    x="X", y="Y"
